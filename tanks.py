...
def trajectory (x0,y0,v,theta,g = 9.8, npts = 1000):
    ...
    vx = np.cos(theta) * v
    vy = np.sin(theta) * v
    tfinal = (vy/g) + np.sqrt(((vy/g)**2)-(2*vy/g))
    t = np.linspace(0,tfinal,num=1)
    x = x0 + (vx*t)
    y = y0 + (vy*t) + ((g * t / 2) ** 2)
    return (x,y)
...
def firstInBox (x,y,box):
    ...
    """
    for j in range(x-1):
        if x[j] >= box[0] and x[j] <= box[1] and y[j] >= box[3] and y[j] <= box[4]:
            return j
        return -1
...
def oneTurn (tank1box, tank2box, obstacleBox, playerNum, g = 9.8):   
    ...
    a = getNumberInput("What is the angle you would like to shoot at? ")
    v = getNumberInput("What is the velocity you would like to shoot at? ")
    if playerNum == 1:
        origin = tank1box
        target = tank2box
    if playerNum == 2:
        origin = tank2box
        target = tank1box
    x0 = (origin[0] + origin[1]) / 2.0
    y0 = (origin[2] + origin[3]) / 2.0
    
    result = tankShot(target, obstacleBox, x0, y0, v, a)
    if result == True:
        return playerNum
    else:
        return 0
...
def playGame(tank1box, tank2box, obstacleBox, g = 9.8):
    ...
    playerNum = 1
    while playerNum != 0:
        result = oneTurn(tank1box, tank2box, obstacleBox, playerNum)
        if result != 0:
            print("Congratulations Player " + playerNum + ", You hit the target!")
            playerNum = 0
        else:
            if playerNum == 1:
                playerNum = 2
